...
import { fetchCart, selectCartItemById, selectGemCartItemById } from '../../slices/cartSlice';
...
import GemVarieties from '../../components/GemVarieties';
import { setLastScreen, setParams } from '../../slices/lastScreenSlice';

export default function GemDetail({ route, navigation }) {
    const { t } = useTranslation();

    const dispatch = useDispatch();
    const accessToken = useSelector((state) => state?.token?.accessToken);
    const [gemId, setgemId] = useState(route?.params?.gemId);
    ...
    const fetchData = async () => {
        try {
            const response = await axios.post(`${uri}/silver/product/detail/`, { silverId: gemId })
    ...
    const SilverVarietyId = data?.varieties?.[activeVariety]?.id;
    const cartItem = useSelector(state => selectGemCartItemById(state, data?.seller_id, SilverVarietyId));

    const quantity = cartItem ? cartItem?.quantity : 0;

    const price = data?.varieties?.[activeVariety]?.get_old_price?.toFixed();
    const discountedPrice = data?.varieties?.[activeVariety]?.get_final_price?.toFixed();
    const discountedPercent = data?.varieties?.[activeVariety]?.discount_percent;
    ...
    const handleIncrease = async () => {
        ...
        try {
            const response = await axios.post(`${uri}/silver/cart/add/`, { SilverVarietyId: SilverVarietyId }, { headers: { 'Accept': 'application/json', 'Authorization': `Bearer ${accessToken}` } })
    ...
    return (
        <SafeAreaView edges={{ top: 'off', bottom: 'additive' }} style={NewStyles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl colors={[themeColor1.bgColor(1)]} refreshing={refreshing} onRefresh={() => { fetchData() }} />}>
                ...
                <ImageBackground style={{ width: '100%', aspectRatio: 1, justifyContent: 'flex-end' }} source={{ uri: `${mainUri}${data?.gallery?.[currentImage]?.image}` }} >
                    {
                        data?.labels?.length > 0 &&
                        <View style={[{ padding: 20, gap: 10 }, NewStyles.row]}>
                            {
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) =>
                        <Pressable onPress={() => setCurrentImage(index)}>
                            <Image style={{ width: deviceWidth * 0.2, height: deviceWidth * 0.2 }} source={{ uri: `${mainUri}${item?.image}` }} />
                        </Pressable>
                    }
                />
                ...
                <View style={styles.wrapper}>
                    ...
                    <View style={[NewStyles.row, { gap: 10 }]}>
                        ...
                        <Text style={NewStyles.text10}>{data?.silver_category?.name}</Text>
                    </View>
                    <GemVarieties data={data?.varieties} activeVariety={activeVariety} setActiveVariety={setActiveVariety} />
                    {data?.silver_category?.show_wage && <View style={[NewStyles.row, { gap: 10 }]}>
                        <Ionicons name="ellipse" size={10} color={themeColor10.bgColor(1)} />
                        <Text style={NewStyles.text10}>اجرت: {data?.varieties?.[activeVariety]?.wage}%</Text>
                    </View>}
                    {data?.silver_category?.show_tax && <View style={[NewStyles.row, { gap: 10 }]}>
                        <Ionicons name="ellipse" size={10} color={themeColor10.bgColor(1)} />
                        <Text style={NewStyles.text10}>مالیات: {data?.tax?.percent}%</Text>
                    </View>}
                    {data?.silver_category?.show_profit && <View style={[NewStyles.row, { gap: 10 }]}>
                        <Ionicons name="ellipse" size={10} color={themeColor10.bgColor(1)} />
                        <Text style={NewStyles.text10}>سود: {data?.varieties?.[activeVariety]?.profit}%</Text>
                    </View>}
            ...
            <View style={[NewStyles.row, styles.box2]}>
                ...
                <View style={{ flex: 2 }}>
                    {quantity == 0 ?
                        <Button title={t('Add To Cart')} loading={pending} disabled={pending} onPress={() => { if (isUserLoggedIn()) { handleIncrease() } else { navigation.navigate('MainLayout', { screen: 'Account' }); dispatch(setLastScreen('Gem Detail')); dispatch(setParams(route?.params)) } }} />
                        :
