...
export default function Sell({ route, navigation }) {
    ...
    const sell = async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.post(`${uri}/bullions/sell`, { amount, detail: description, product_id: item?.id }, { headers: { 'Accept': 'application/json', 'Authorization': `Bearer ${accessToken}` } })
    ...
    return (
        <SafeAreaView edges={{top:'additive', bottom:'additive'}} style={[NewStyles.container,styles.contentContainerStyle]}>
            <View style={NewStyles.center}>
                <Text style={NewStyles.heading10}>درخواست فروش</Text>
            </View>
            ...
            <Button title={'ثبت درخواست'} loading={loading} onPress={() => sell()} />
        </SafeAreaView>
    )
}
