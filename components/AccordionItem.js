...
export default function AccordionItem({ item, index, active, setActive }) {
    return (
        <Pressable style={[styles.itemWrapper, NewStyles.shadow, NewStyles.border10]} onPress={() => { if (active == item?.id) { setActive() } else { setActive(item?.id) } }}>
            <View style={[NewStyles.rowWrapper, { gap: 10 }]}>
                <Text style={NewStyles.title}>{index + 1}</Text>
                <Text style={[NewStyles.text10, { flex: 1 }]}>{item?.question}</Text>
                <Ionicons name={active == item?.id ? "chevron-up" : "chevron-down"} size={15} color={themeColor0.bgColor(1)} />
            </View>
            {(active == item.id) && <Text style={[NewStyles.text10, { textAlign: 'justify', direction: 'rtl' }]}>{cleanText(item?.answer)}</Text>}
        </Pressable>
    )
}
