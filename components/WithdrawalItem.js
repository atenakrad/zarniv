...
export default function WithdrawalItem({ item }) {
    ...
    const accountTitle = item?.bank_account?.title || item?.bank_account?.bank_name || item?.bank_name;
    ...
    return (
        <View style={[styles.itemWrapper, NewStyles.shadow, NewStyles.border10]}>
            ...
            {accountTitle && renderRow('حساب مقصد', accountTitle)}
            {item?.iban && renderRow('شماره شبا', item.iban)}
            {item?.card_number && renderRow('شماره کارت', item.card_number)}
            {item?.bank_account_number && renderRow('شماره حساب', item.bank_account_number)}
