...
export default function GemItem2({ item, navigation }) {

    const { t } = useTranslation();
    const image = item?.gallery?.[0];
    const price = Number(item?.old_price)?.toFixed(0);
    const discountedPrice = Number(item?.old_price)?.toFixed(0) - (Number(item?.old_price)?.toFixed(0) * Number(item?.discount_percent) / 100);
    const discountedPercent = item?.discount_percent;

    return (
        <Pressable style={[styles.contentWrapper, NewStyles.row, NewStyles.shadow, NewStyles.border5]} onPress={() => navigation.navigate('Gem Detail', { gemId: item?.id })}>
            {image ? <Image style={styles.content} source={{ uri: `${mainUri}${image?.image}` }} contentFit='cover' /> : <View style={styles.content}><ImageThumbnail height={200} /></View>}
